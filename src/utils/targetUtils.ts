 ...
 *    - BBL / BBLS / blank / unknown: Ziyadah target only (e.g. "Zi 15 Baris")
 */
export function getEffectiveTargets(
  ...
  if (isManual) {
    if (
      participant?.target_lines !== undefined &&
      participant?.target_lines !== null &&
    ...
    if (
      halaqah?.target_ziyadah_lines !== undefined &&
  ...
  // 2. Nuroniyyah target calculation (always in BARIS)
  let nuroniyyahLines: number | null = null;

  const getParticipantNur = (): number | null => {
    if (
      participant?.target_nuroniyyah_lines !== undefined &&
      participant?.target_nuroniyyah_lines !== null &&
  ...
  const getHalaqahNur = (): number | null => {
    if (
      halaqah?.target_nuroniyyah_lines !== undefined &&
  ...
  const skill = participant?.skill_status_start
    ? String(participant.skill_status_start).trim().toUpperCase()
    : '';
 ...
 * - Nur +8 Baris
 * - Iqra +3 Halaman
 * - Zi +5 Baris • Nur +8 Baris • Iqra +3 Halaman
 */
export function formatSplitProgressDisplay(student: {
  ...
  const zi =
    Number(
      student.totalZiyadahLinesAdded ??
      (
        student.totalNuroniyyahLinesAdded !== undefined ||
        student.totalIqraPagesAdded !== undefined
          ? 0
          : student.totalLinesAdded ?? 0
      )
    ) || 0;

  const nur =
    Number(student.totalNuroniyyahLinesAdded) || 0;

  const iqra =
    Number(student.totalIqraPagesAdded) || 0;

  const parts: string[] = [];

  if (zi > 0) {
    parts.push(`Zi +${zi} Baris`);
  }

  if (nur > 0) {
    parts.push(`Nur +${nur} Baris`);
  }

  if (iqra > 0) {
    parts.push(`Iqra +${iqra} Halaman`);
  }
