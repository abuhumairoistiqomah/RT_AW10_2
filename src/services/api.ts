...
const STORAGE_KEYS = {
  ...
  PENDING_WRITES: 'rt_lms_pending_writes',
...
export function getCircuitBreakerState() {
  ...
  const remainingCooldownSeconds = isCooldownActive
    ? Math.ceil((CIRCUIT_BREAKER_COOLDOWN_MS - (Date.now() - circuitBreaker.lastFailureTime)) / 1000)
    : 0;
...
export function recordCircuitSuccess(): void {
  if (circuitBreaker.failureCount > 0 || circuitBreaker.status !== 'ONLINE') {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('rt_backend_status_change', {
        detail: { status: 'ONLINE', failureCount: 0 }
      }));
    }
  }
}
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent('rt_backend_status_change', {
      detail: {
        ...
        failureCount: circuitBreaker.failureCount,
        error: error?.message || 'Network transport failure'
      }
    }));
  }
}
  ...
  try {
    ...
    if (err?.name === 'AbortError' || controller.signal.aborted) {
      ...
      throw new NetworkError(
        `Batas waktu koneksi habis (${seconds} detik). Google Apps Script sedang lambat atau mengalami cold start. Silakan coba kembali.`,
        true
      );
    }

    throw new NetworkError(
      `Koneksi jaringan terputus atau server tidak dapat dihubungi: ${err?.message || 'Network request failed'}`
    );
  } finally {
    clearTimeout(timer);
  }
...
async function apiPost<T>(
  ...
  try {
    ...
    const error = new NetworkError('Format respons server tidak valid (bukan JSON).');
...
function readPendingWritesLocal(): PendingWriteItem[] {
  try {
    const raw = localStorage.getItem(STORAGE_KEYS.PENDING_WRITES);
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? parsed : [];
  } catch (e) {
    console.error('[Pending Sync] Failed reading queue:', e);
    return [];
  }
}
...
function notifyPendingWritesChanged(items?: PendingWriteItem[]): void {
  ...
  window.dispatchEvent(new CustomEvent('rt_pending_sync_change', {
...
function writePendingWritesLocal(items: PendingWriteItem[]): void {
  try {
    localStorage.setItem(STORAGE_KEYS.PENDING_WRITES, JSON.stringify(items));
    notifyPendingWritesChanged(items);
  } catch (e) {
    console.error('[Pending Sync] Failed saving queue:', e);
  }
}
...
function enqueuePendingWrite(action: PendingWriteAction, payload: any, dedupeKey: string): PendingWriteItem {
  ...
  const idx = queue.findIndex(item => item.dedupeKey === dedupeKey);
...
function removePendingWrite(id: string): void {
  writePendingWritesLocal(readPendingWritesLocal().filter(item => item.id !== id));
}
...
function removePendingAttendanceForAssessment(assessment: SessionAssessment): void {
  ...
  readPendingWritesLocal().forEach(item => {
    if (
      item.action !== 'bulkSaveSessionAttendance' ||
      String(item.payload?.sessionConfigId || '') !== sessionConfigId
    ) {
      next.push(item);
      return;
    }
    ...
    if (remainingIds.length) {
      next.push({
        ...item,
        payload: { ...item.payload, studentIds: remainingIds },
        dedupeKey: `attendance:${sessionConfigId}:${remainingIds.slice().sort().join(',')}`,
        updatedAt: getCurrentIso()
      });
    }
  });
...
function applyAttendanceOverlay(a: SessionAssessment, attendanceStatus: string): SessionAssessment {
  ...
  if (status === 'PRESENT') {
    return { ...a, attendance_status: 'PRESENT', assessment_status: a.assessment_status || 'PENDING' } as SessionAssessment;
  }
...
function overlayPendingWritesOnAssessments(
  serverAssessments: SessionAssessment[],
  context?: {
    eventId?: string;
    halaqahId?: string;
    sessionConfigs?: SessionConfig[];
    students?: Array<{ student_id: string; participant_id: string }>;
  }
): SessionAssessment[] {
  const result = [...(serverAssessments || [])];
  const queue = readPendingWritesLocal().slice().sort(
  ...
  const upsert = (asm: SessionAssessment) => {
    ...
    const idx = result.findIndex(x =>
      !x.is_deleted &&
      x.event_id === asm.event_id &&
      x.participant_id === asm.participant_id &&
      x.session_config_id === asm.session_config_id
    );
    if (idx >= 0) result[idx] = { ...result[idx], ...asm };
  ...
  queue.forEach(item => {
    ...
    studentIds.forEach(studentId => {
      ...
      if (idx >= 0) {
        result[idx] = {
          ...applyAttendanceOverlay(result[idx], attendanceStatus),
          ...({ __pendingSync: true } as any)
        };
        return;
      }
      ...
      result.push({
        assessment_id: `LOCAL_PENDING_${sessionConfigId}_${studentId}`,
        event_id: context?.eventId || sc.event_id,
        event_day_id: sc.event_day_id,
        session_config_id: sessionConfigId,
        participant_id: st.participant_id,
        student_id: studentId,
        halaqah_id: context?.halaqahId || '',
        session_no: sc.session_no,
        attendance_status: attendanceStatus as AttendanceStatus,
        assessment_status: attendanceStatus === 'PRESENT' ? 'PENDING' : 'COMPLETED',
        teacher_id: '',
        is_deleted: false,
        created_at: now,
        updated_at: now,
...
function applyPendingOverlayToWorkspace<T extends Record<string, any>>(workspace: T): T {
  ...
  const merged = overlayPendingWritesOnAssessments(
    ws.assessments || ws.sessions || [],
    {
      eventId: ws.event?.event_id,
      halaqahId: ws.halaqah?.halaqah_id,
      sessionConfigs: ws.sessionConfigs || [],
      students: ws.students || []
    }
  );

  const students = (ws.students || []).map((student: any) => {
    const asms = merged.filter(a =>
    ...
    asms.forEach(a => {
      ...
      else if ((a as any).iqra_level || (a as any).iqra_page_start || (a as any).iqra_page_end) {
        iqra += Number((a as any).iqra_pages_added) || 0;
    ...
    return {
      ...student,
      totalLinesAdded: zi,
      totalZiyadahLinesAdded: zi,
      totalNuroniyyahLinesAdded: nur,
      totalIqraPagesAdded: iqra
    };
  });
...
export class ApiService {
  static async warmBackend(): Promise<void> {
    ...
    if (!baseUrl || baseUrl.includes('YOUR_DEPLOYMENT_ID')) return;

    try {
      const url = `${baseUrl}${baseUrl.includes('?') ? '&' : '?'}action=health`;
      ...
      console.debug('[Backend Warmup] GAS belum siap, request nyata akan mencoba lagi.', error);
    }
  }
  ...
  static async testConnection(customUrl?: string): Promise<{ connected: boolean; message: string; data?: any }> {
    ...
    try {
      ...
      if (json?.success && json?.data?.spreadsheetConnected === true) {
        recordCircuitSuccess();
      ...
      return {
        connected: false,
        message: json?.error?.message || 'Spreadsheet tidak terhubung atau Web App belum siap.'
      };
    } catch (e: any) {
  ...
  static async checkHealth(): Promise<{ connected: boolean; message: string; lastChecked?: string }> {
    ...
    return {
      ...
      message: r.connected ? 'Google Sheets Terhubung' : (r.message || 'Database Tidak Terhubung'),
  ...
  static async flushPendingWrites(): Promise<{ syncedCount: number; remainingCount: number; failedCount: number }> {
    if (isMockMode) {
      writePendingWritesLocal([]);
      return { syncedCount: 0, remainingCount: 0, failedCount: 0 };
    }
    ...
    pendingFlushPromise = (async () => {
      ...
      if (syncedCount > 0) {
        ...
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('rt_backend_data_changed', {
            detail: { source: 'pending-sync', syncedCount }
          }));
        }
      }
  ...
  static async getSessionAssessments(eventId?: string): Promise<SessionAssessment[]> {
    ...
    return eventId
      ? list.filter(a => a.event_id === eventId && !a.is_deleted)
      : list.filter(a => !a.is_deleted);
  }
  ...
  static async saveSessionAssessment(asm: SessionAssessment, actorUserId?: string): Promise<SessionAssessment> {
    if (!isMockMode) {
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('rt_backend_data_changed', {
            detail: {
              ...
              studentId: asm.student_id,
              sessionConfigId: asm.session_config_id
            }
          }));
        }
        return saved;
        ...
        if (isRetryableSyncError(error)) {
          console.warn('[Pending Sync] Assessment disimpan lokal dan akan disinkronkan ulang.', error);
          return { ...asm, ...({ __pendingSync: true } as any) };
        }
        throw error;
      }
    }
    ...
    if (idx >= 0) {
      ...
      await this.addAuditLog(
        'UPDATE_ASSESSMENT',
        'SESSION_ASSESSMENT',
        updatedItem.assessment_id,
        JSON.stringify(old),
        JSON.stringify(updatedItem),
        undefined,
        actorUserId || asm.teacher_id,
        asm.event_id
      );
      return updatedItem;
    }
  ...
  static async bulkSaveSessionAttendance(
    ...
    if (!isMockMode) {
      ...
      if (!ids.length) return { success: true, updatedCount: 0, updatedAssessments: [] };

      enqueuePendingWrite(
        'bulkSaveSessionAttendance',
        { sessionConfigId, studentIds: ids, attendanceStatus },
        `attendance:${sessionConfigId}:${ids.slice().sort().join(',')}`
      );

      void this.flushPendingWrites().catch(error =>
        console.warn('[Silent Attendance] Background sync tertunda:', error)
      );
  ...
  static async getMyHalaqahData(teacherId: string, eventId?: string, selectedHalaqahId?: string) {
    ...
    if (!isMockMode) {
      const server = await apiPost<any>('getMyHalaqahData', { teacherId, eventId, selectedHalaqahId });
      return applyPendingOverlayToWorkspace(server);
    }
  ...
  /**
   * Final Evaluation — SINGLE REQUEST WRITE
   * Production uses participant_id already carried by teacher workspace.
   * No getCurrentEvent/getEvents/getEventParticipants preflight before the save.
   * Code.gs resolves event/student/halaqah and UPSERTs by event_id + participant_id.
   */
  static async submitFinalEvaluation(payload: any, actorUserId?: string): Promise<FinalEvaluation> {
    ...
    if (!isMockMode) {
      ...
      if (!participantId) {
        throw new Error('participant_id tidak tersedia pada workspace. Muat ulang data halaqah lalu coba simpan kembali.');
      }

      const finalEvaluation: any = {
        participant_id: participantId,
        student_id: payload.student_id || undefined,
        evaluation_surah_start: sStart != null && sStart !== '' ? Number(sStart) : undefined,
        evaluation_ayah_start: aStart != null && aStart !== '' ? Number(aStart) : undefined,
        evaluation_surah_end: sEnd != null && sEnd !== '' ? Number(sEnd) : undefined,
        evaluation_ayah_end: aEnd != null && aEnd !== '' ? Number(aEnd) : undefined,
      ...
      if (payload.final_evaluation_id) {
    ...
    const eventId = currentEvt?.event_id;
    if (!eventId) throw new Error('Event aktif tidak ditemukan.');

    const participant = (await this.getEventParticipants(eventId)).find(p =>
    ...
    const existing = (await this.getFinalEvaluations(eventId)).find(e =>
      e.participant_id === participant.participant_id || e.student_id === participant.student_id
    );
    const now = getCurrentIso();

    const fe = {
      final_evaluation_id:
        existing?.final_evaluation_id ||
        payload.final_evaluation_id ||
        `FEVAL-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`,
      event_id: eventId,
      participant_id: participant.participant_id,
      student_id: participant.student_id,
      evaluation_surah_start: sStart != null && sStart !== '' ? Number(sStart) : undefined,
...
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('online', () => {
    void ApiService.warmBackend();
    void ApiService.flushPendingWrites().catch(error =>
      console.warn('[Pending Sync] Auto-sync setelah online gagal:', error)
    );
  });
}
